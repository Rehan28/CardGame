...
public class Game{
    ...
    public void play(){
        ...
        do {
            keepPlayingUntilIncrrGuess();
            gamePlayed++;
            playAgain = askUserToPlayAgain();

        }while (playAgain);
        double averageScore = ((double) correctGuesses)/gamePlayed;
        System.out.println("You played "+gamePlayed+" game");
        System.out.printf("Your average score was %1.3f.\n",averageScore);
    }
    private void keepPlayingUntilIncrrGuess(){
        Deck deck = new Deck();
        deck.shuffle();
        Card currentCard = deck.dealcards();
        Card nextCard;
        ...
        while (keepPlaying){
            ...
            if(nextCard.getValue() == currentCard.getValue()){
                System.out.println("The value is the same as the privious card.");
    ...
    private boolean askUserToPlayAgain(){
        ...
        return input.nextLine().toUpperCase(Locale.ROOT).contains("YES");
    }
    private char askForUserInput(){
        System.out.println("Will the next card be higher (H) or Lower (L) : ");
        char guess;
        do {
            guess = input.nextLine().toUpperCase(Locale.ROOT).charAt(0);
            if(guess != 'H' && guess !='L'){
                System.out.println("Place type H or L !");
            }
